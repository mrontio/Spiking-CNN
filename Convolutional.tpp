...
Tensor* Convolutional::forward(const Tensor& input)
{
        ...
        for (shape k = 0; k < channels_out_; ++k) {
                for (shape l = 0; l < channels_in_; ++l) {
                        for (shape i  = 0; i < input_shape[2]; i += stride_) {
                                for (shape j  = 0; j < input_shape[3]; j += stride_) {
                                        auto output_idx = TensorShape{k,i,j};
                                        (*output)[output_idx] += apply_kernel(k, l, i, j);
                                }
                        }
                }
        }
...
inline float Convolutional::apply_kernel(shape k, shape l, shape i, shape j) {
        ...
        for (shape m = 0; m < weights_.shape(2); ++m) {
                ...
                for (shape n = 0; n < weights_.shape(3); ++n) {
                        shape n_rel = n - 1;
                        auto weight_idx = TensorShape{k,l,m,n};
                        auto pad_idx = TensorShape{l, i_pad + m_rel, j_pad + n_rel};
                        output += weights_[weight_idx] * padding_buffer_[pad_idx];
                }
        }
        return output;
}
